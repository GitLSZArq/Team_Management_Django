...
function Tasks({ tasks = [], projects = [] }) {
  ...
  const handleRightClick = (event, item) => {
    ...
    setTaskDetails(item);
    console.log('Selected task details for editing:', item);
    setEditMode(false);
  };
  ...
  const handleSave = async () => {
    ...
    // Extract numerical part from task ID
    const numericTaskId = taskId.match(/\d+/)[0];
    console.log('Numeric task ID:', numericTaskId);

    // Convert task ID to integer
    taskId = parseInt(numericTaskId, 10);
    console.log('Converted task ID to integer:', taskId);
    ...
    console.log('Saving task to URL:', url);
