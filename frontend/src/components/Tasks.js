...
function Tasks({ tasks = [], projects = [], setTasks }) {
  ...
  const [visibleProjects, setVisibleProjects] = useState(projectList);

  // NEW: Controlled state for selected projects in the filter
  const [selectedProjects, setSelectedProjects] = useState([]);

  // Define the ProjectFilter component:

  const ProjectFilter = ({ projects, selected, onFilterChange, onClose }) => {
    // Toggle a single project.
    const toggleSelection = (projectId) => {
      if (selected.includes(projectId)) {
        onFilterChange(selected.filter((id) => id !== projectId));
      } else {
        onFilterChange([...selected, projectId]);
      }
    };
    ...
    const toggleSelectAll = () => {
      if (selected.length === projects.length) {
        ...
        onFilterChange(projects.map((p) => p.id));
      }
    };
    ...
    return (
      ...
      >
        ...
        {projects.map((project) => (
          <label key={project.id} style={{ display: 'block', marginLeft: '10px' }}>
            <input
              type="checkbox"
  ...
  // Define a handler for filter changes:
// 3) Handler for applying the filter (called from inside ProjectFilter)

  const handleFilterChange = (newSelected) => {
    setSelectedProjects(newSelected);
    if (newSelected.length === 0) {
      // If no projects are selected, show all projects.
      setVisibleProjects(projectList);
    } else {
      // Only show projects whose IDs are in newSelected.
      setVisibleProjects(projectList.filter((proj) => newSelected.includes(proj.id)));
    }
  };
  ...
  const RenderTaskItem = ({
    ...
    const onMouseDown = (e) => {
      if (e.button === 1) { 
        // 🟢 Middle-click ONLY: Toggle drag mode
        e.preventDefault(); 
        toggleDragEnabled(item.id);
      } else if (e.button === 0) {
        // 🔴 Left-click: Store initial click position (to detect dragging)
    ...
    const onMouseUp = (e) => {
      if (e.button === 0) { 
        // 🔴 Left-click release: Detect if it's a real click or drag attempt
        if (clickStart) {
          ...
          if (distance < 5) {
            // ✅ Small movement = it was just a click, NOT a drag!
            if (isProject) {
        ...
        setClickStart(null); // Clear state
      }
    };
        ...
        <div className="timeline">
          ...
          >
            <TimelineHeaders className="sticky">
              <SidebarHeader>
                {({ getRootProps }) => (
                  <div {...getRootProps()} className="sidebar-header" style={{ position: 'relative' }}>
